  ...
  new Vue({
    ...
    methods: {
      getVueItems: function() {
        this.$http.get('/paciente/getDataPaciente').then((response) => {
          if(response.data.length == 0){
            var message = 'No hay Pacientes registrados';
            $.notify({
                icon: 'fa fa-warning',
    ...
    moreItems: function () {
      ...
      filtro: function() {
          if (this.filter.filtro == null || this.filter.filtro == "") {
              ...
              this.$http.post('/paciente/getDataFilterPaciente',filtro).then((response) => {
          console.log(response.data);
          if(response.data.length == 0){
            var message = 'No hay Pacientes registrados con estos caracteres'
            $.notify({
                icon: 'fa fa-warning',
                title: '<strong>Información!</strong>',
                message: message
